...
const OurOfferingFirstComeFirstServe = ({item, canRedeem}) => {
    ...
    const buyTokenPackages = () => {
        initDialog({
            ...
            description: `${item.percent} of ${item.percentOf} Sold`,
    ...
    return <>
        <Grid item xs={12}>
            <Box className={'box-green p-m-0-m'}>
                <div>
                    <Typography className={'font-16 light-font'}>{strings.ourOfferings.timeToGuaranteedAllowlistSaleEnd} </Typography>
        ...
        <Grid item md={6} xs={5}>
            <Button disabled={!canRedeem} variant={'outlined'} className={'full-size-button white-black-button btn-md bordered w-100 m-xs font-20'} onClick={() => openArtworkPage()}>{strings.ourOfferings.findOutMore}</Button>
        </Grid>
        <Grid item md={6} xs={5}>
...
export default OurOfferingFirstComeFirstServe;